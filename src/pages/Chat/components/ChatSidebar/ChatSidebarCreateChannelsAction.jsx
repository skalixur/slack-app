...
export const ChatCreateChannelForm = () => {
  const { setUserChannels, userChannels, allUsers } =
  ...
  function handleSelectUser(user) {
    setSelectedUsers((prev) =>
      selectedIds.has(user.id) ? prev : [...prev, user],
    )
  }
  ...
  const filteredUsers = allUsers.filter(
    (user) => user.uid !== uid && !selectedIds.has(user.id),
  )
    ...
    <form className='flex flex-col gap-2' onSubmit={handleFormSubmit}>
      <div className='grid grid-cols-3 grid-rows-2 items-center gap-2 text-right'>
        ...
        {selectedUsers.length > 0 && (
          <h3 className='text-md p-2 text-left font-semibold'>Added users</h3>
        )}
